...
/*   Updated: 2024/12/14 17:00:25 by abablil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename Container>
void PmergeMe::print(int ac, char **args, Container &container, const std::string &name, clock_t time)
{
	std::cout << "Before: ";
	for (int i = 1; i < ac; i++)
		std::cout << args[i] << " ";
	...
	std::cout << std::endl;
	std::cout << "Time to process a range of " << ac - 1 << " elements with " << name << " : " << time << "ms" << std::endl;
}
...
void PmergeMe::runVector(int ac, char **args)
{
	...
	time = clock() - time;
	print(ac, args, biggerNumbers, "std::vector", time);
}

void PmergeMe::runDeque(int ac, char **args)
{
	...
	this->startSorting(biggerNumbers, lowerNumbers, oddNumber);
	time = clock() - time;
	print(ac, args, biggerNumbers, "std::deque", time);
}